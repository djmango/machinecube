...
export async function generateChildComponents(componentName: string, ancestry: string[]): Promise<Omit<Component, 'parent'>[]> {
  ...
  - Focus ONLY on physical, tangible components that would appear in a bill of materials
  - Each component should be a real, manufacturable part or assembly
  - Use proper engineering/manufacturing terminology
  - Consider standard part hierarchies (assembly → subassembly → component → part)
  ...
  const userPrompt = `Generate exactly two physical subcomponents or parts that would be direct children in a bill of materials for "${componentName}" ${ancestryString}.
  These should be real, tangible parts that could be manufactured or sourced.`;
